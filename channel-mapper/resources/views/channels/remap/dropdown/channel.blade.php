        <a href="#" class="dropdown-item channel-remap-select {{ $active ? 'active' : '' }}" data-channel-number="{{ $channel->GuideNumber ?? $channel->Number }}" data-channel-search="{{ $channel->GuideNumber ?? $channel->Number }} {{ Str::upper($channel->CallSign ?? '') }} {{ Str::upper($channel->GuideName ?? '')}} {{ Str::upper($channel->Name ?? '') }} {{ Str::upper($channel->Title ?? '') }}" role="button">
